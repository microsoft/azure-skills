...
- Skills directory -> `config.skills_dir` only when the runtime has a skill/tool loading mechanism or one is explicitly added. If defaults should include local file-based skills, explicitly load them before `config.compose_instructions()`; `default_skills_dir` alone only records the fallback location.
- Tool definitions -> `config.tool_definitions` only when the framework exposes safe tool metadata surfaces, such as function docstrings or schema descriptions.
...
- environment-variable fallback support for `OPTIMIZATION_CONFIG`; keep `AGENT_OPTIMIZATION_CONFIG` as backward compatibility only
- optional candidate resolver support for `OPTIMIZATION_CANDIDATE_ID` and `OPTIMIZATION_RESOLVE_ENDPOINT`
- optional local candidate directory support via `OPTIMIZATION_LOCAL_DIR`, with relative paths resolved from the main entrypoint directory
...
Do not introduce alternate `FAOS_OPTIMIZATION_*` environment variable names in the generated package unless the user explicitly asks for a compatibility adapter. For hosted agents, prefer non-reserved `OPTIMIZATION_*` variables in user-authored config; keep `AGENT_OPTIMIZATION_CONFIG` only for backward compatibility with older inline-config injectors.
...
Do not automatically add every optimization env var to `agent.yaml`. Hosted agent vNext reserves platform-owned `AGENT_*` variables in deployment payloads, so never add `AGENT_OPTIMIZATION_*` there. If the workflow needs local candidate files or inline fallback config, add only non-reserved variables such as `OPTIMIZATION_LOCAL_DIR` or `OPTIMIZATION_CONFIG` and keep optional resolver variables documented unless explicitly required.
